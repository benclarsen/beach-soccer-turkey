...
# for actual analyses of sensitive data (not available on GitHub)
#output_dir <- "results" 
#project_name <- "beach_soccer"
...
### Raw case data --------
#caselist <- read_delim("data/caselist_beach_soccer.csv", delim = ";")
caselist <- read_delim("data/caselist_example.csv", delim = ";")

### Exposure data --------
data_exposure <- data.frame(
  match = 271,
  training = 8501
) %>%
  mutate(total = match + training)

player_days <- 5815  # Total player-days observed

### Load OSIICS v15 -------
my_locale <- locale(encoding = "UTF-8")
osiics_15 <- read_delim("data/osiics_15.csv", delim = ";", locale = my_locale)
